...
Taxonomy MITRE is joined to **`mitre_permitted[]`** on `question_runtime_map_v1.json` (emit via `coverage_drafter --emit-maps`). Default live `/chat` still uses legacy use-case KB mapping before canonical cutover (historical, non-runtime); the control-plane flag-on path can merge registry + use-case KB overlap in [`mitre_permitted.py`](../../backend/app/threat/mitre_permitted.py) until Phase 7 runtime MITRE decisioning lands.
