...
def match_use_cases(query: str, *, limit: int = 3) -> list[UseCaseSelection]:
    ...
    for use_case in load_use_case_catalog():
        matched = [pattern for pattern in use_case.intent_patterns if pattern.lower() in normalized]
        if not matched:
            continue
        confidence = min(0.95, 0.68 + (0.06 * len(matched)) + _intent_boost(normalized, use_case.use_case_id))
        matches.append(
            UseCaseSelection(
