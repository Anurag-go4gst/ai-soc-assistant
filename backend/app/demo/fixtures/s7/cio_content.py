...
S7_CIO_CONTENT = CioContent(
    ...
    executive_brief={
        "investigation": {
            "verdict": "Real incident: OT-RTU-14 is live and was accessed by vendor account ot_vendor_svc with no maintenance window. The CMDB record saying it was retired is wrong.",
            ...
            "decision_needed": "Approve: open an incident, narrow the firewall path used by the vendor account, check the RTU configuration, and correct the CMDB.",
            "will_not_do": "Power down or isolate the RTU without OT engineering — that could interrupt the substation.",
        },
        "complete": {
            "verdict": "Incident open: vendor account ot_vendor_svc accessed a live RTU without a maintenance window. Path narrowed, RTU configuration clean, CMDB corrected.",
            "business_impact": "Substation operation uninterrupted. The device is back under monitoring and patch management, and the vendor path is closed.",
            "risk_from": "HIGH",
            "risk_to": "MEDIUM",
            "confidence": "High — source, liveness and configuration all verified.",
            "would_change_if": "The vendor cannot account for the session, or the account shows use elsewhere → escalate to a confirmed OT compromise.",
            "decision_needed": "Approve a review of vendor account ot_vendor_svc with the vendor within 24 hours.",
            "will_not_do": "Close the incident before the vendor review is complete.",
        },
    },
)
