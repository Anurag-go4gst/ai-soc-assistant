...
def test_bridge_client_timeout_is_hard_capped(monkeypatch) -> None:
    ...
    member = LocalChatClient(
        base_url="http://qwen/v1",
        model="./qwen72b",
        timeout_seconds=120,
    )
    monkeypatch.setattr(
        "app.llm.clients.endpoint_resolver.build_failover_chat_client",
        lambda **kwargs: FailoverChatClient(chain=(("local_primary", member),)),
    )
    capped = bridge._bridge_client()
    assert capped is not None
    assert capped.chain[0][1].timeout_seconds <= bridge._BRIDGE_TIMEOUT_SECONDS_CAP
...
def test_live_evidence_planning_never_calls_llm_inline(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.llm.clients.endpoint_resolver.build_failover_chat_client", _boom
    )
