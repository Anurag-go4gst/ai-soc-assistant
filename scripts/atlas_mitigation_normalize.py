#!/usr/bin/env python3
"""ATLAS mitigation rows → normalized JSON (plan 2026-07-06 item 4)."""
...
NORMALIZATION_RULES_VERSION = "atlas-mitigation-normalize-v1"
...
def build_report() -> dict:
    ...
    with CSV_PATH.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            if str(row.get("entity") or "").strip() != "mitigation":
                continue
            text = str(row.get("text") or row.get("description") or "")
            rows.append(
                {
                    "mitigation_id": str(row.get("id") or "").strip(),
    ...
    rows.sort(key=lambda item: item["mitigation_id"])
    return {
        "schema_role": "atlas_mitigations_normalized_v1",
...
def main() -> int:
    ...
    out = NORMALIZED_DIR / "atlas_mitigations_normalized.json"
    out.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(f"ATLAS mitigations: {report['mitigation_count']} -> {out.relative_to(ROOT)}")
    return 0
