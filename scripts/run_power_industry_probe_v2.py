...
BANK_PATH = REPO_ROOT / "docs" / "evals" / "power_industry_probe_v2_bank.json"
OUT_JSON = REPO_ROOT / "docs" / "evals" / "power_industry_probe_v2_results.json"
OUT_MD = REPO_ROOT / "docs" / "evals" / "power_industry_probe_v2_report.md"
...
def _extract(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload_spl = bool(spl.get("spl") or spl.get("query") or spl.get("candidate_spl") or preview.get("draft_spl") or validation.get("normalized_spl"))
    card_spl = bool(analyst.get("draft_spl_code") or analyst.get("spl_code") or analyst.get("spl_draft_preview") or contract.get("render_sections", {}).get("spl_artifact") or "draft spl" in lowered_message or "candidate spl" in lowered_message)
    return {
    ...
    with sentinel_runtime():
        for entry in bank["entries"]:
            ...
            try:
                ...
                observed["signal_class"] = classify_signal_class(question)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"total": result["total"], "quality_flags": result["quality_flags"], "quality_violations": result["quality_violations"]}, indent=2))
    return 1 if args.check and result["quality_violations"] else 0
