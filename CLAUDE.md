...
| `plans/2026-07-01_1545_guided-readonly-mcp-discovery-lane.md` | **Draft** — split guided_investigation's single `mcp_allowed` boolean into execution (stays off) vs a read-only MCP discovery lane (5 metadata tools: `splunk_get_info`/`indexes`/`metadata`/`index_info`/`knowledge_objects`); `run_query`+SPL-gen stay gated. New `EvidencePlan.discovery_allowed` + loop-entry gate `mcp_allowed OR discovery_allowed`, assessor caps guided to `discovery_hop`/`finalize` (never `execute`). Promotion = **HIL offer, not auto route change** (out-of-registry has no governed contract to promote to). Hops via existing Stage-4B spine → canonical RunContract (planned excluded from `collected_evidence_count`). Behind default-off COE flag `AI_SOC_GUIDED_MCP_DISCOVERY_ENABLED`; flag-off byte-identical; only guided family touched. Also corrects details.html §9/§12 overstatement (guided has no planned discovery today). |
