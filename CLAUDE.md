...
- SPL generation: relevance-first — governed templates → LLM-primary failover (`AI_SOC_LLM_SPL_FALLBACK_ENABLED`, default off) → lab draft last resort. Lab-tier LLM SPL always `approved=false`/`normalized_spl=null`. `graph_node_spl_source_resolve` fills placeholders from `AI_SOC_SOURCE_PROFILE_MAP`/RAG/session pins, else HIL clarification. See `docs/architecture/spl_generation_audit.md`.
