...
**Canonical active plan — reference this one only:**
...
(loop runner: [`plans/LOOP_RUNNER_guided-detail-tools-consumable-handoff.md`](plans/LOOP_RUNNER_guided-detail-tools-consumable-handoff.md)).
All canonical-planning / dual-runtime-parity work is tracked there. Other plans below are
historical or scoped elsewhere; do not treat them as the current work item.

Most-relevant in-flight/recent plans:

| Plan | Status |
|------|--------|
| `plans/2026-07-24_2310_guided-detail-tools-consumable-handoff.md` | **In Progress (rev 13) — CANONICAL ACTIVE PLAN.** T0–T4 canonical planning cutover. Gate 1 **complete** (items 12/13/14): sentinel 17/17, clean-answer 120/120, `base_105_loaded=105`, clarification uses typed `CanonicalPlanningOutcome` with no partial `EvidencePlan`. Item 30 done (parity root cause); item 31 done (projection, three classifications, approval ineligibility, dead `_ACCEPTABLE_DIFF_FIELDS` removed). Production parity baseline `120 total / 113 exact / 0 approved / 7 critical_mismatch` (`imperative_canonical` vs `resource_planner_graph`) — supersedes the legacy `planner_led_shadow_graph` measurement. Next: item 32 (shared `run_canonical_planning` seam, 7 Category G rows). Full pytest 111 failed / 4233 passed |
