...
- Chat control plane (query-to-intent, evidence planning, route adjudication, SPL slot binding, MITRE decision, `control_plane_trace`) runs unconditionally via canonical planning on every `/chat` turn. See `plans/2026-06-02_chat-control-plane-master.md` (historical flag-gate removed in item 25 cutover). Plan 5 B: understanding is a typed `ResolvedQueryContract` (no skill, no execution authority) emitted before `adjudicate_route`; the provisional route is not an understanding input. Bounded T4 semantic understanding and live route-level capability enforcement both stay **default OFF** (`ai_soc_t4_semantic_understanding_enabled`, `ai_soc_live_capability_enforcement_enabled`). Required capabilities are satisfied by the complete governed schedule, not necessarily by one primary skill (B5: `cisco.ot.029` is why route-level "one skill must grant everything" was rejected). Map: `docs/architecture/routing_authority_map.md`.
...
- Scheduling authority is **built and default-off** (Plan 5 C, Plan 3 A0 `PHASE_POLICY_PLUS_RESOURCE_PLAN_SCHEDULING`): closed `PhaseRegistry` + deterministic `PhasePolicy` resolver + per-run immutable `PhaseContract`; ResourcePlan + PhaseContract merge into one dependency-valid schedule (`planner/phase_schedule_merge.py`). Lifecycle phases are mandatory-when-applicable, not universal. `AI_SOC_RESOURCE_PLAN_EXECUTION_ENABLED` stays **default false**; flag-off runs zero merge-seam code. Dispatch-v2 projected schedules still win when present. Flag-on probe closed the Plan 3 A0 4-of-5 stage-drop (`merged_stage_drops=0/5`). No `DECISION_REQUIRED` seam adopted; `_run_legacy_dispatch_fallback` not retired (it still skips `spl_postprocessor`; MCP gate refuses unapproved/null `normalized_spl`). `mitre_finalize`/`cve_adapter` still execute inside `graph_node_context_finalize` rather than the hook loops. See `docs/architecture/phase_contract_and_schedule.md`. **COE:** `AI_SOC_PIPELINE_DISPATCH_V2_ENABLED` is false at repo default and true on this VPS. **Which committed profile supplies it is not `coe.env.example`:** compose loads `env/profiles/${AI_SOC_ENV_PROFILE:-coe}.env.example` and then `.env`, and this host sets `AI_SOC_ENV_PROFILE=development` (Plan 6 F4), so `development.env.example` is the file in effect. Read the deployment's `AI_SOC_ENV_PROFILE` before reasoning about any flag, and note that a rollback editing only `.env` will not drop profile-supplied keys.
...
| Plan | Status |
|------|--------|
| `plans/2026-08-14_1130_resource-plan-authority-and-t4-integration.md` | **Active — Plan 7.** Opened by Plan 6 `F5 = DEFER`. Makes `ResourcePlan + PhaseContract + deterministic compiler` production-authoritative, retires dispatch-v2 from normal authority, fixes the missed `spl_postprocessor` work **structurally**, runs the target corpus with **T4 ON**, then remediates T4 serving and re-gates go-live. Remediation posture: exec **ON**, v2 **OFF**, T4 **ON** @ 2.0 s, live capability enforcement **OFF**. No keyword heuristics; "one primary skill owns all capabilities" stays rejected. |
| `plans/2026-08-13_1440_production-activation-t4-serving-and-governance-readiness.md` | **Done (37/37) — Plan 6, PR #132, unmerged.** `P6_PRODUCTION_GO_LIVE = **DEFER**` — architecture proven **experimentally**, not production-authoritative; go-live not declared while authority is dispatch-v2. `GO LIVE` was available (blockers **0**) and deliberately not taken. C0 **KEEP OFF**, C3 **0 seams**, D3 **T4 2.0 s / default-off**, E2 MITRE **deferred**, E3 reports **preserved**, E4 **0** protected members. Blockers handed to Plan 7: with v2 OFF, `p6.multi.knowledge_spl_mcp` / `p6.live_posture.d1_003` lose `spl_postprocessor`; T4 serving non-viable (9/9 timeouts, no alternate endpoint, host swap-full). MCP is **`live_mcp_unproven`** — no live rows claimed. Repo defaults unchanged. Report: `docs/evals/plan6_activation_and_t4_report.md` |
