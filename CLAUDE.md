...
| Plan | Status |
|------|--------|
| `plans/2026-08-12_1230_production-readiness-understanding-phase-contract.md` | **Plan 5 — Active, closing (D1/E0/G0; G1 remaining).** Baseline `2f678b9`. A: runtime-map builder byte-idempotent, MITRE containment preserved, protected manifest **15/15**, 11-row DRAFT promotion deferred. B: `ResolvedQueryContract`; provisional route removed from understanding; T4 semantic hop and live route-level capability enforcement **default OFF**; B5 rejected "one skill must grant everything" (`cisco.ot.029`). C: `PhaseRegistry` + `PhasePolicy` + `PhaseContract` + merge seam; execution flag stays false; no seam adopted; fallback not retired. D0: residual 25 rows, L4=L5 **10 resolved / 15 unchanged / 0 regressed**. D1: `RATIFIED_FOR_MEASURED_ROWS` (7 live-posture rows) + `DEFERRED_T4_SEMANTIC_SERVING_LIMIT` (8 paraphrases). Report: `docs/evals/plan5_architecture_and_routing_report.md` |
