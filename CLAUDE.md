...
| Plan | Status |
|------|--------|
| `plans/2026-07-04_1736_intent-mcp-tool-routing-hardening.md` | Draft (item 0 done: alias-dedupe/stale-comment/await_execution spine fixes, suite 3985 green) — Phase 1: playbook intent hints, data-silence advisory HIL, saved-search name allowlist + saved-search-first preference, kv_store surface-pending. Phase 2: governed evidence observation (`evidence_observer` reads bounded sanitized MCP rows, grounded claims, governed ReAct, 1-call/turn cap). Phase 3: canonical reference-knowledge path (CVE/ATT&CK/ATLAS registry, `reference_taxonomy` shape, `reference_finalize`, 10-probe contract incl. negatives, LLM shape advisor main-path advisory, zero-hallucinated-ID quality evals) |
