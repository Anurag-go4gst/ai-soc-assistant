...
- Pattern→skill hints (`LEGACY_ROUTER_INTENT_BY_PATTERN`): `notable_risk_lookup`, `case_state_lookup` and `threat_intel_enrichment` route to `attack_discovery` (Plan 4 R2.1) — they are live-data retrieval whose labels require SPL, which `alert_summary`/`knowledge_recall` deny. **`asset_identity_context` and `data_source_health` are deliberately unresolved**: 10 truth-set rows, all `capability_inconsistent`, all marked ambiguous and non-gating. Do not settle them by widening a skill contract.
- **The D2 "39-row out-of-registry fallback" framing is disproved** (Plan 4). Measured against independent labels, **38 of 39** rows are already correctly routed — `knowledge_recall` is right for those guidance/policy/out-of-scope asks. D2 is a **3-row** defect (`rt.d2.003/010/017`) and is **not fixed**: no deterministic signal isolates those rows (two have an empty signal set), so a discriminating rule needs a new classifier.
- **Largest known routing defect is paraphrase handling** (Plan 4 E0, not fixed): 10 of the 13 remaining `capability_inconsistent` rows and 10 of the 12 remaining wrong routes are paraphrased hunt questions collapsing to `knowledge_recall`; only 1 of 15 paraphrases reaches a near/semantic-105 path. Invisible to the 105 goldens by construction.
- `tools/coverage_authoring/question_runtime_map_builder.py` is **not idempotent**: re-running it nulls all 11 MITRE registry fields on all 105 rows of `question_runtime_map_v1.json`, because a later commit promoted that metadata and the builder does not reproduce it. Do not regenerate that map without restoring MITRE fields; carried as `RUNTIME_MAP_BUILDER_IDEMPOTENCY`.
...
| Plan | Status |
|------|--------|
| `plans/2026-08-11_1834_routing-evaluation-and-authority-corrections.md` | **Done (19/19), merged PR #130 @ `2f678b9` — current authoritative baseline.** Built the independent routing truth set (87 rows, blind-labelled, 20/20 inter-labeller agreement on both gating axes) — the repo previously had no instrument that could tell a correct route from an incorrect one. **D3** restored deterministic finality over the LLM advisory (capability downgrades 5→0, divergences 10→5). **R2.1** corrected three pattern→skill classes (`d1` quota 0/8→8/8 route-ok). **D2's 39-row premise disproved** — a 3-row defect, unfixed, no safe discriminator. Overall route-correct **56/77 → 64/76**, `capability_inconsistent` **21 → 13**, live route-correct **51 → 59/76**, unsafe containment unchanged. Known gaps, all recorded: paraphrase routing (largest remaining), 3 SPL semantic misses, deferred `asset_identity_context`/`data_source_health` ownership, `UNDERSTANDING_BEFORE_FINAL_ROUTE` (unapproved), `RUNTIME_MAP_BUILDER_IDEMPOTENCY`. Report: `docs/evals/routing_evaluation_report_v1.md` |
| `plans/2026-08-11_0915_execution-driven-adoption-and-guided-refinement.md` | **Done (9/9).** H0 live `query_signals` degrade fix; A0 = `PHASE_POLICY_PLUS_RESOURCE_PLAN_SCHEDULING` (decided, **not built** — phase contract needs its own plan); A1 seam-coverage inventory + structural pins (10 paths: 2 SEAM / 4 DECISION_REQUIRED / 4 KEEP_SEPARATE, **0 adopted**); B0 bounded guided refinement wired to real evidence/gap state; B1 flag OFF/ON **neutral**, default stays false; B2 generic `routed_skill × intent_family × contract` capability compatibility, fail-closed, consumed by **both** Phase Policy and composition. Closure: governance PASS, backend `5046 passed`, parity `120 exact`, probes 10/10 + 11/11, manifest 13/13, invariant 7/7. Deferred + unapproved: `UNDERSTANDING_ROUTER_ON_LOW_CONFIDENCE` (99/105 goldens hit the 0.2-confidence `knowledge_recall` default). |
