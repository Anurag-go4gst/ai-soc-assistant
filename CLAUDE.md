...
**Most recent completed authority-migration plan:**
[`plans/2026-08-14_1130_resource-plan-authority-and-t4-integration.md`](plans/2026-08-14_1130_resource-plan-authority-and-t4-integration.md)
(**Plan 7 CLOSED 25/25**, merged). **Active:** Plan 8 [`plans/2026-08-15_0602_canonical-architecture-authority-convergence.md`](plans/2026-08-15_0602_canonical-architecture-authority-convergence.md). Index: [`plans/README.md`](plans/README.md).

Most-relevant in-flight/recent plans:

| Plan | Status |
|------|--------|
| `plans/2026-08-15_0602_canonical-architecture-authority-convergence.md` | **Active — Plan 8.** Frozen `architecture.md` is read-only. Converge final RQC, ResourcePlan authority, EvidenceState, InvestigationOutcome, exact-call SPL authorization, trust boundaries, and T4 reliability. Production GO remains deferred. F3 T4 serving and live MCP stay unproven. PlanDelta / step-instance execution remain `NOT_REQUIRED_FOR_CURRENT_SCOPE`. |
...
| `plans/2026-06-29_conditional-pipeline-canonical-dispatch.md` | **Historical.** All 12 phases shipped behind `AI_SOC_PIPELINE_DISPATCH_V2_ENABLED` (repo default **false**). Plan 7 retired v2 from **normal** authority; this host's `development` profile keeps it off. Bounded pre-SPL discovery remains a different mechanism from the retired planning-model loop. Check `AI_SOC_ENV_PROFILE` before reasoning about SPL inputs. |
