...
| Item 29 policy-boundary corrective | **Done** | `9fa2798` — unsafe containment → `policy_blocked_outcome` (discovered during Item 29 smoke; owned by canonical policy/outcome architecture; **not** a second Item 21 telemetry delivery) |
...
- [x] **29** — Containerised `/chat` canonical smoke — rev 9 (**Done 2026-07-26 @ `9fa2798` smoke source**)
  ...
  - **Evidence (prior run superseded):** Earlier direct-backend 6/6 claim invalidated by Nginx requirement + policy contract gap + turn-2 `response_validation_failed`.
  - **Evidence (2026-07-26 close @ smoke source `9fa2798`):**
    - **Policy-boundary corrective (`9fa2798`, Item 29 discovery):** `canonical_policy_boundary.py` + orchestrator `policy_blocked_outcome` branch; `test_canonical_policy_blocked_live_routing.py`. Distinct from **Item 21 telemetry catalog** (`8cd2c2d`, 28-event durable catalog). Prior Item 29 stop @ `a32b1d2` exposed the routing gap; corrective landed before harness KEEP commit.
    - **Harness:** `scripts/smoke_canonical_paths.sh` + `smoke_canonical_paths_runner.py`; Nginx overlay `docker-compose.smoke.yml` + `docker/nginx-smoke.conf`. `_canonical_status()` derives from `plan_dispatch.canonical_status`, `canonical_planning_outcome`, committed `evidence_plan.resource_plan`, or `resource_plan.created` DB event — **never** from `dispatch_source=legacy_predicate` (trace label inside canonical `execute_plan_dispatch`, not legacy planning authority; `rg legacy_predicate backend/app` → executor schedule builder only).
    - **Idempotency (item 20 contract-aware):** per-probe `PROBE_IDEMPOTENCY_EXPECTATION` — `read_only_dispatch` probes allow `execution.*` without `canonical_execution_idempotency` rows; `none_policy_terminal` for `policy_blocked`; invariants enforced when rows exist (no duplicate plan/step keys).
    - **Nginx `--check` 6/6 @ `9fa2798`:** `SMOKE_RUN_ID=item29-close-9fa2798`; all probes via `http://127.0.0.1:18041/api` (Nginx). `t1_known_complete` 98.4s canonical=planned idem=read_only_dispatch events=13 handoffs=v1:plan_committed conn_peak=0; `t1_clarification_resume` 147.0s turn1 clarification.requested+handoff.persisted (no request.completed), turn2 canonical=planned handoffs=v1:resumed,v2:plan_committed events=23 one resource_plan.created; `t3_near_semantic_match` 246.9s; `t4_guided_resolution` 122.3s; `t0_knowledge_only` 212.4s; `policy_blocked` 4.6s canonical=policy_blocked hr=unsafe_action_blocked handoffs=none events=8 exactly one request.completed forbidden execution/clarification/resource_plan. All conn_peak=0 ≤ budget 5.
    - **Regression slice @ `9fa2798`:** focused policy/planning/telemetry/architecture **105 passed**; full pytest **4517 passed / 0 failed**; integration **34 passed / 0 skipped**; sentinel **17/17** (`pg.unsafe.001.contract_answer_mode` → `null` reflects policy-boundary corrective); clean-answer **120/120**; production parity **120 exact / 0 approved / 0 critical**.
  - **Evidence (2026-07-26 resume @ `a32b1d2`, STOPPED — superseded):**
    - **Step 1 — clarification.requested:** Turn-1-only live probe via Nginx-isolated stack confirms production emits/persists `clarification.requested` + `handoff.persisted` with typed correlations (`handoff_id`, `handoff_version`, `decision_id`, `trace_id`). No `request.completed`, `execution.started`, or `resource_plan.created` on turn 1. Example turn-1 IDs: `trace_id=0c914103-4223-4a57-9965-410c1cbf962f`, `handoff_id=cpi:57657431ca8a` v1, `session_id=smoke:canonical:item29-resume-1785078848:t1_clarification_resume:e64f439dec`. Prior harness false-negative was timing/query scope — harness fix (trace-scoped poll) in uncommitted `scripts/smoke_canonical_paths_runner.py`. **No Item 21 corrective** for missing `clarification.requested`.
    - **Step 2 — policy_blocked:** **Contract/routing gap.** No committed canonical policy test provides a live query asserting `canonical_status=policy_blocked`. `test_canonical_planning_outcomes.py::test_outcome_policy_blocked` constructs `policy_blocked_outcome()` only. `test_canonical_telemetry_coverage.py::test_policy_unsafe_block` uses `"Block IP 10.0.0.5 immediately"` but asserts only `execution.started` absent — live Nginx probe returns `canonical_status=clarification_required`, `human_review.reason=unsafe_action_blocked`. `policy_blocked_outcome()` has **zero production callers** (`rg policy_blocked_outcome backend/app --glob '!**/tests/**'` → none). Smoke harness now requires strict `policy_blocked` — probe **FAIL** (1.8s).
    - **Step 3 — Nginx:** Added `docker-compose.smoke.yml` + `docker/nginx-smoke.conf`; `AI_SOC_PUBLIC_API_BASE_URL` → `http://127.0.0.1:18031/api`. `/health` via Nginx: migrations 0001–0006 ready. Source SHA `a32b1d2`.
    - **Step 5 — partial probes:** `t1_clarification_resume` turn 1 **PASS** (harness poll); turn 2 **FAIL** — `request.failed` / `response_validation_failed` after `resource_plan.created` (HTTP 200). `policy_blocked` **FAIL** (canonical status gap). **Did not run `--check` 6/6.** Harness remains **uncommitted** per KEEP gate.
    - **Next:** (a) Item 21 production work to wire unsafe blocks → `policy_blocked_outcome` + committed live query, or relax smoke contract with COE approval; (b) ~~fix turn-2 `response_validation_failed` on clarification resume~~ **Corrective Item B done** — see addendum below; (c) harness idempotency assertion may need alignment when execution runs without idempotency rows.
  - **Corrective Item B (2026-07-26):** Turn-2 `response_validation_failed` / `clarification_suppressed` on clarification resume.
    - **Root cause:** Turn-1 clarification persisted `routing.intent_family=clarification_required` and `routing.answer_goal=clarification`. Resume reconstruction reused those stubs instead of restoring from `original_skill` (`alert_summary`), so `plan_evidence_from_canonical` committed a plan with `evidence_plan.answer_mode=clarification` and empty `clarification_questions` → `validate_final_response` failed with `clarification_suppressed` while HTTP stayed 200.
    - **Latency (143s probe):** Turn-2 planning/execution ~300ms; total probe time dominated by turn-1 + turn-2 **LLM final-synthesis** gaps (`response.validated` → `response.generated` ~120s each) — not a resume-path defect.
    - **Fix:** `canonical_query_to_intent_resume.py` — restore `intent_family` from `original_skill`, normalize answer goals (`live_investigation` → `live_results`); `canonical_planning_orchestrator.py` — preserve pre-clarification `original_answer_goal` from gap before intent stub mutation.
    - **Verify:** `pytest app/tests/test_canonical_clarification_contract.py -q` → **19 passed**; Nginx live two-turn probe → `request.completed`, `execution.started/completed`, no `request.failed` (trace `13ee70c3-…`, 265s total, ~121s+141s synthesis).
