...
- [x] **P0 — Freeze the pre-change baseline and record the observed starting state**
  ...
  - **Evidence:** **CLOSED 2026-08-09 on `master@6ec8c9c`.**
    ...
    **specialist_reports cardinality (the A1 defect, measured):** 64 reports / 4 unique lanes = **16x** on rag_only, SPL and guided paths alike (`skill/knowledge/mcp/spl` = 16 each). Mechanism: `Annotated[list, operator.add]` at `resource_planner_graph.py:143` plus every node returning full state via `_record`; masked by `assert len(reports) >= 4` at `test_resource_planner_graph_skeleton.py:118`.
...
- [ ] **A1 — Stop additive specialist-report amplification**
  ...
  - **Depends on:** A0
  - **Evidence:** **READY, NOT STARTED.** The user confirmed the parallel documentation session ended; `master` is reconciled and the freshly captured manifest checks clean for all 13 protected artifacts with no ignore exemption. When done, record exact cardinality for every probe and confirm no specialist lane removed.
