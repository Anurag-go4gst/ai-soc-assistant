...
| 2026-07-24 | Ship: `feat/resource-planner-north-star` pushed to origin; **PR #107** open (base `master`, 12 commits) — "feat(planner): Resource Planner north-star cutover (RP default, runner retirement)". |
| 2026-07-24 | Live probe (uvicorn `:8010`, backend restarted, session-auth): `POST /api/auth/login` authenticated; `POST /api/chat` → **200**; `selected_skill=knowledge_recall`; note tail `…Orchestration: resource_planner_hierarchy.` (`resource_planner_hierarchy=true`, `(parity mode)=false`); `control_plane_trace` present (COE `.env` runs `control_plane_enabled=true`, operator override per plan). In-process TestClient same query = same RP note signals (~1.8s vs ~67s live full COE stack). Live `/chat` serves RP graph, not linear parity. |
