...
- [x] **G2** — Final closure gates
  ...
  - **Depends on:** G1
  - **Evidence:** No authority or `config.py` default moved in Plan 6, so the forecast-rule set ran **once** plus a re-audit of every checkmark. Governance regression **PASS**; backend pytest **5281 passed, 3 skipped, 6 xfailed, 0 failed** (Plan 5 baseline 5247 — delta is Plan 6's new tests); truth-set `--check` **PASS**, 64/76, **0 regressions**; production parity **total=120 base_105=105 exact=120 approved=0 critical=0**; Cisco **PASS=50 REVIEW=0 FAIL=0 CRITICAL=0**; sentinel **17/17**; path honoring **105/105**; protected manifest **15/15**; `/invariant-check` **7/7**; plan-discipline audit **0 gaps**. Six stale reports reverted after governance (`git checkout` on exactly those six; never `docs/evals/` wholesale). No unrelated dirt committed. F5 `P6_PRODUCTION_GO_LIVE = DEFER` present in Approved decisions. PR **#132** open with all STOP decisions recorded; **not merged**, merge SHA absent.
  - **Reference probes — recorded as DRIFT, not 10/10.** `audit_reference_probes.py --check` reports drift on all ten (every field reads `-> None`: empty probe responses in this host environment, not a changed route). **Pre-existing, not a Plan 6 regression** — the same probes at the Plan 5 baseline `1d32ac6` in a clean worktree produce identical drift and slightly worse (`P3 selected_skill: knowledge_recall -> spl_generation`, extra `N2` field). The frozen contract was captured in a different environment than this host now runs. Baseline deliberately **not** refreshed: converting this into a green 10/10 would be exactly the unfalsifiable gate the script's own docstring warns about. Carried to Plan 7 as an environment/gate-fidelity item.
