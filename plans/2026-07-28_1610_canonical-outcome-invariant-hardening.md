...
- [ ] One valid typed outcome after shared seam
- [ ] No dispatch without valid `planned`
- [ ] Missing/malformed/contradictory fail closed
- [ ] RP ≡ imperative terminal status on non-planned and corrupt cases
- [ ] Clarification: no EP/RP; policy/persistence unchanged
- [ ] Planned rag/SPL routing unchanged
- [ ] P1–P7 migrated; full pytest + governance + parity 120/0/0 + sentinel 17/17 + clean-answer 120/120
...
- [x] **C1** — Tri-state reader + cross-state validators
  - **Evidence:** `pytest app/tests/test_canonical_outcome_read.py app/tests/test_canonical_planning_outcomes.py -q` → 18 passed

- [x] **C2** — Pure typed failure builder + strict allowlist
  - **Evidence:** `rg emit_request_failed backend/app/chat/canonical_mode.py` → empty; `pytest app/tests/test_canonical_outcome_gate.py app/tests/test_canonical_clarification_contract.py -q` → 29 passed

- [x] **C3** — Shared invariant gate
  - **Evidence:** wired in `run_canonical_planning`; `pytest app/tests/test_canonical_outcome_gate.py app/tests/test_canonical_policy_blocked_live_routing.py -q` → 8 passed

- [x] **C4** — Migrate P1–P7 to tri-state reader
  - **Evidence:** `pytest app/tests/test_dual_runtime_single_orchestration.py app/tests/test_response_validation_canonical.py -q` → 27 passed

- [x] **C5** — Legacy clarification removal + outcome_from_state doc
  - **Evidence:** `rg 'answer_mode.*clarification' backend/app/chat/pipeline.py backend/app/chat/pipeline_dispatch_builder.py` → no dispatch branches; docstring updated

- [x] **C6** — Behavioural parity + negative controls + disposable compose IT
  - **Evidence:** baseline `workflow_spl` without outcome → after fix `non_planned_finalize`; `pytest app/tests/test_dual_runtime_behavioural_parity.py app/tests/test_canonical_outcome_invariant_negative_controls.py -q` → 16 passed; `DATABASE_URL=@127.0.0.1:5544 pytest app/tests/integration/ -q` → 34 passed 0 skipped

- [x] **C7** — Full gates + docs (workstream B; C deferred)
  - **Evidence:** governance PASS; parity `120/0/0`; clean-answer `120/120`; `git diff --check` clean; config comment fixed; completion report §15 gap-2 addendum

## Verification gaps

_None at plan authoring — re-run audit before C1 implementation._
...
**COMPLETE (workstreams A+B).** Ready for PR assembly — no push/deploy in this loop. Workstreams C/D/E deferred.
