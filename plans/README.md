...
| Plan | Status |
|------|--------|
| [`2026-08-12_1230_production-readiness-understanding-phase-contract.md`](2026-08-12_1230_production-readiness-understanding-phase-contract.md) | **Active, closing (25/28 checked; G1 remaining).** Plan 5 on baseline `2f678b9`. **A** runtime-map builder byte-idempotent; MITRE containment preserved; destructive authoring test isolated; protected manifest **15/15**; 11-row DRAFT promotion deferred. **B** `ResolvedQueryContract`; provisional route removed from understanding; tier vocabulary unified; T4 semantic hop and live route-level capability enforcement **default OFF**; B5 lesson: required caps are satisfied by the complete governed schedule (`cisco.ot.029`). **C** `PhaseRegistry` + `PhasePolicy` + `PhaseContract` + merge seam; execution flag stays false; no seam adopted; fallback not retired. **D0** residual 25 rows: L1 0/25/0, L4=L5 **10/15/0**. **D1 closed:** `D1_LIVE_POSTURE_ROUTE = RATIFIED_FOR_MEASURED_ROWS` (7 measured rows, not a family-wide rule); `D1_PARAPHRASE_RESIDUE = DEFERRED_T4_SEMANTIC_SERVING_LIMIT` (8 T4 paraphrases). Report: [`docs/evals/plan5_architecture_and_routing_report.md`](../docs/evals/plan5_architecture_and_routing_report.md). Stale governance reports remain unrefreshed by instruction. |
