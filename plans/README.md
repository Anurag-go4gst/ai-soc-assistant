...
| Plan | Status |
|------|--------|
| [`2026-08-12_1230_production-readiness-understanding-phase-contract.md`](2026-08-12_1230_production-readiness-understanding-phase-contract.md) | **Active (0/25, approved 2026-08-12 with amendments)** — Plan 5, production-readiness architecture on baseline `2f678b9`. **A** fixes the runtime-map builder (measured: **7** promoter-owned MITRE fields dropped and **2** conflicting on 61/105 rows — not "11 nulled"; map lives at `backend/app/coverage/`, not `docs/evals/`), then adds it to the protected manifest (14 → 15). **B** introduces a typed `ResolvedQueryContract`; the real defect is **circularity, not ordering** — understanding already precedes final adjudication (`pipeline.py:2341`), but `build_query_to_intent` consumes the provisional `routed_skill` (`canonical_planning_orchestrator.py:447/:520`). **C** builds Plan 3 A0's decided architecture as three distinct artifacts — **PhaseRegistry + deterministic PhasePolicy resolver + per-run PhaseContract** — plus the deterministic merge seam; lifecycle phases are mandatory **when deterministically applicable**, not universally, and PhasePolicy (never the LLM or planner) owns applicability. **D** re-measures the residual routing defects *after* B+C rather than patching them. Recorded decisions: A1 `DRAFT_AUTHORITATIVE_CURRENT_BEHAVIOR`, A5 protection approved post-proof, B2 `lane_router` tier vocabulary, B5 capability veto **default-OFF** (activation not approved), C3 proof-only (no seam adopted, fallback not retired), D1 deferred. Two facts that shape execution: capability compatibility is **not enforced on a default live turn** today, and the second execution engine is dormant at repo default but **live on the COE host**. |
| [`2026-08-11_1834_routing-evaluation-and-authority-corrections.md`](2026-08-11_1834_routing-evaluation-and-authority-corrections.md) | **Done (19/19, 2026-08-12) — merged PR #130 @ `2f678b9`, the current authoritative baseline.** Closure gates: governance regression PASS, backend `5119 passed / 0 failed`, parity `120 exact`, Cisco 50/50, harness 6/6, probes 10/10, out-of-set PASS, truth-set `--check` 0 regressions, manifest 14/14, invariants 7/7. — Plan 4. Built the independent routing truth set (87 rows, blind-labelled; 20/20 inter-labeller agreement on both gating axes) because the repo had no instrument that could tell a correct route from an incorrect one — the 105 goldens are exact-match, circular, and matched production routing on 1/105. **D3** restored deterministic finality over the LLM advisory (capability downgrades 5→0). **R2.1** corrected three pattern→skill classes (`d1` 0/8 → 8/8). **D2's 39-row premise disproved** — a 3-row defect, unfixed, no safe deterministic discriminator. Route-correct **56/77 → 64/76**; live **51 → 59/76**. `UNDERSTANDING_ROUTER_ON_LOW_CONFIDENCE` **retired as a measured no-op**. Open, all recorded: paraphrase routing (largest remaining defect), 3 SPL semantic misses, deferred `asset_identity_context`/`data_source_health` ownership, `UNDERSTANDING_BEFORE_FINAL_ROUTE` (unapproved), `RUNTIME_MAP_BUILDER_IDEMPOTENCY`. Report: [`docs/evals/routing_evaluation_report_v1.md`](../docs/evals/routing_evaluation_report_v1.md) |
