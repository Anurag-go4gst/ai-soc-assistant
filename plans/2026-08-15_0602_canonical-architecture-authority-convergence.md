...
- [x] **SPL0 — Audit current SPL entity, lifecycle, validation, and authorization ownership**
  ...
  - **Evidence:** Nine-answer map attached to `docs/architecture/canonical_architecture_audit_2026-08-15.md` § Plan 8 SPL0. Verify `rg` → **4715** lines. Classes: extraction `PARTIAL` (no dest-IP/domain/geo field); RQC store `PARTIAL`; `workflow_spl` does not consume final RQC (`PARTIAL`); `spl_source_resolve` `EXISTS` (slots only); `spl_postprocessor` `EXISTS` (hygiene, A3 preserved); `validate_spl` `EXISTS`; RQC-constraint survival `MISSING`; MCP gate `EXISTS`; call-binding `PARTIAL` (replay fingerprint only). No runtime change. `architecture.md` unmodified.

- [x] **SPL1 — Preserve final-RQC entities and mandatory constraints through normalized SPL**
...
- [x] **G0 — Run targeted, broad, governance, and novel-query gates**
