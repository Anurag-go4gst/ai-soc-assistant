...
- [x] **B1 — Controlled flag OFF vs ON evaluation**
  ...
  - **Evidence:** **COMPLETE 2026-08-11 — no default change proposed.** Bounded in-process matrix, 10 probes × 2 arms (control flag OFF, candidate flag ON). No live `/chat` arm, per the recorded method assumption.
    ...
    **Finding that matters more than the deltas.** The two probes that did **not** activate both downgraded with `no_schedulable_step`. End-to-end tracing dispositioned them differently — the first was my own harness's fault:

    | Probe | Disposition | Detail |
    |---|---|---|
    | T1 alert summary | **`PROBE_ARTIFACT` — RETRACTED** | The harness forced `routed_skill="alert_summary"`. The real router sends that query to `attack_discovery`, where the plan is consistent (`spl_artifact, mcp_execution, narration`, no vetoes). No contradiction exists on the reachable path, and the locked alert-summary/no-SPL protection is intact, enforced by `canonical_answer_mode_policy`'s `alert_summary_spl_contradiction`. |
    | novel OT | **`ROUTE_INTENT_CONTRADICTION`** (secondary `VALIDATION_POLICY_CONFLICT`) | Router-reachable. `needs_spl=True` with a contract-blocked `mcp_execution` + `narration`; the keyword router's 0.2-confidence `knowledge_recall` default collides with `spl_generation_only`. |

    For the real case, plan-derived scheduling concludes "nothing schedulable" while predicate-derived scheduling builds a full SPL lane — the two surfaces disagree about the turn's work. The downgrade masked it; under a compiler-authoritative model it would become dropped work. **Closed at the capability layer by B2**; the router default that causes it is deferred and unapproved.
    ...
    **RE-RUN after B2 (2026-08-11):** same matrix, unchanged result — 10 probes, 9 composed, **7 activated, 0 schedule differences**. No regression, no safety or authority change, so no stop condition fired. Note the matrix runs with dispatch-v2 **off in both arms**, so it does not exercise B2's Phase Policy constraint; B2's effect was verified separately by parity (120 exact) and its own targeted tests.

    **Disposition of the two `no_schedulable_step` probes (see B2):** the T1 alert-summary case is **retracted as `PROBE_ARTIFACT`** — the harness forced `routed_skill="alert_summary"` for a query the real router sends to `attack_discovery`, where the plan is consistent and no contradiction exists. The novel-OT case is confirmed a real, router-reachable **`ROUTE_INTENT_CONTRADICTION`** with secondary **`VALIDATION_POLICY_CONFLICT`**, root-caused to the keyword router's unmatched default (`knowledge_recall` @ 0.2 confidence, contract forbids SPL/MCP) colliding with `spl_generation_only`. Closed at the capability layer by B2; the router default itself is deferred.
...
- [x] **B2 — Routed-skill × intent-family capability compatibility (fail closed, no route change)**
  - **Do:** Add a deterministic compatibility contract resolving `routed_skill + intent_family + skill capability contract` into resolved capabilities, classifying `COMPATIBLE` / `CAPABILITY_CONTRADICTION` / `PROTECTED_CONTRADICTION` / `UNRESOLVED`. Fail closed when the contradiction changes execution capability (intent requires SPL/MCP + contract forbids it) — never widen capability to rescue the route. Both Phase Policy and ResourcePlan composition must consume the same resolved constraints; composer/validator vetoes remain defense-in-depth. Do **not** change the unmatched router default.
  - **Why:** B1 exposed a router-reachable contradiction where the two planning surfaces disagreed about the same turn and nothing reconciled them.
  - **Surfaces:** `backend/app/chat/skill_intent_compatibility.py` (**NEW**); `backend/app/chat/pipeline_dispatch_builder.py`; `backend/app/tests/test_skill_intent_compatibility.py` (**NEW**).
  - **Depends on:** B1 disposition.
  - **Failing-first / observation:** Phase Policy must emit a full SPL lane for the OT contradiction before the fix, and none after. Negative controls must keep the lane for a capability-granting skill.
  - **Verify:** `cd backend && PYTHONPATH=../backend:.. python3 -m pytest app/tests/test_skill_intent_compatibility.py app/tests/test_pipeline_dispatch_phase6b.py app/tests/test_dispatch_authority_wiring.py app/tests/test_skill_contract_planning.py -q`; production parity `--check`; reference + out-of-set probes `--check`; full backend pytest; manifest `--check`.
  - **Evidence:** **COMPLETE 2026-08-11; runtime commit `70c3090`.** New `skill_intent_compatibility.py` + 16 tests. Resolution against **real registry contracts**: `attack_discovery × spl_generation_only` → `COMPATIBLE` (spl granted); `knowledge_recall × spl_generation_only`, `alert_summary × spl_generation_only`, `guided_investigation × spl_generation_only` → `CAPABILITY_CONTRADICTION` with `denied={spl}` and **capability never widened**; `knowledge_recall × knowledge_only` → `COMPATIBLE`; unknown intent family and missing contract → `UNRESOLVED` (never asserted safe).

    **Interpretation recorded explicitly:** "fail closed" constrains the **capability**, not the turn. Measured on the golden set, **14/105 turns already sit in this contradiction class** (8 `knowledge_recall`, 6 `alert_summary`, all `spl_generation_only`) and **none of their accepted answers contains SPL** — the lane ran and contributed nothing. Denying the capability removes wasted work; erroring those turns would have regressed 14 accepted answers for no safety gain.

    **Phase Policy is no longer contract-blind:** `_apply_capability_constraints` drops SPL/MCP lanes the resolved contract forbids and is pinned to **only ever remove** stages, never add one. Capability lookup **delegates to the composer's own `_skill_permits`** rather than duplicating the hint table, so both surfaces answer "does this skill allow SPL?" from one implementation — a second copy would be a second authority by another name.

    **Failing-first proof:** stashing the Phase Policy wiring fails exactly 3 tests (`..._emits_no_spl_lane_for_the_ot_contradiction`, `..._only_removes_stages`, `..._leaves_the_schedule_untouched`), 13 pass; restored → **16 passed**.

    **Gates.** Targeted **76 passed** across compatibility + dispatch phases 4/5/6b/8 + authority wiring + skill-contract planning + seam coverage. **Production parity `total=120 base_105=105 exact=120 approved=0 critical=0`** — answer-neutral, exactly as the golden-set evidence predicted. Reference probes **10/10**; out-of-set **11/11**. **Full backend `5046 passed, 2 skipped, 6 xfailed, 0 failed`.** Manifest 13/13. Invariant **7/7 PASS** — capability can only be denied; MCP gate, HIL, RBAC and SPL validator untouched.
  - **Invariant / manifest:** Full invariant check; capability may only narrow.
  - **Commit boundary:** Compatibility contract + Phase Policy consumption only; no router default change.
  - **Stop:** Any parity regression, or a fix requiring the unmatched router default to change.

- [x] **G0 — Align documentation with the selected outcomes**
...
### `UNDERSTANDING_ROUTER_ON_LOW_CONFIDENCE` — NOT APPROVED, NOT IMPLEMENTED

The keyword router's unmatched default is `knowledge_recall` at **0.2 confidence** (`LOW_CONFIDENCE_ROUTE`, reason *"insufficient evidence to select a specialized skill"*). Because that skill's contract forbids SPL and MCP, every unmatched query inherits a no-SPL contract — which is what manufactures the contradiction class B2 now closes at the capability layer.

Measured evidence: **99/105** golden questions hit that default; **91 of those 99** receive a different route from the understanding router; its distribution is `attack_discovery` **83**, `alert_summary` **8**, `knowledge_recall` **8**.

Hypothesis to test later: *keyword rule matched with sufficient confidence → keep the deterministic keyword route; low-confidence / no rule matched → use the existing deterministic understanding-router result.*

Because this would change routing for **~94%** of the golden set, it requires its own OFF/ON routing matrix plus parity and answer-quality evaluation before adoption. It is **not** authorized by Plan 3, and B2 deliberately did not touch the default.
...
| 2026-08-11 | **B1 COMPLETE — no default change proposed.** 10 probes × 2 arms, dispatch-v2 forced off in both so the compiler could activate (7 of 9 composed probes activated). **Zero schedule differences**; flag ON is neutral. Recorded caveat: production runs v2 ON, where the compiler never activates, so this measures the two schedulers rather than production impact. **Finding beyond the deltas (as recorded at the time — the T1 half was SUPERSEDED the same day):** two non-activating probes downgraded `no_schedulable_step`. End-to-end tracing then showed the **T1 alert-summary half was a `PROBE_ARTIFACT` and is retracted** — the harness forced a routed skill the real router never selects, and on the reachable path (`attack_discovery`) the plan is consistent with no contradiction. Only the **novel-OT** case is real: a router-reachable `ROUTE_INTENT_CONTRADICTION` where `needs_spl=True` meets a contract-blocked `mcp_execution` plus `narration`. Closed at the capability layer by B2. Flag stays default false. |
