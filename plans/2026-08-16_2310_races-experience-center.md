...
- [x] **G5** — Commit / PR (merge only if user asked)
  ...
  - **Depends on:** G4
  - **Evidence:** G5.1–G5.5 on `6c88828` / evidence commit `e17a3e5`. Draft PR **#143** marked ready and **merged** 2026-08-17. Merge SHA **`d4f9210303aac5f6ae964f0259fb9fc25dd59743`**. Post-merge smoke on that master: GET `/demo/scenarios` **10/10** legacy IDs; GET `/demo/experience-center/scenarios` **7 Flagship + Lab** (24); S1 run `production_side_effect=false`; S5 `approve_upgrade=APPROVED` → `execute_upgrade=EXECUTED` → `verify_version=VERIFIED` (version 15); `/chat` parity-off uses production pipeline (`test_live_chat_ec_parity_off_uses_pipeline`); live `/chat` hit LLM+fallback (not EC). `/scenarios` 200, bundle has Flagship + Lab labels. Plan `status: done`.

## Pre-G5 review (2026-08-17)

Independent review found three correctness issues after D–G. Closed in R1–R4. **G5 checked 2026-08-17.** PR #143 merged to `master` @ `d4f9210`. Plan status `done`.
...
- 2026-08-17 G5: Pushed `feat/races-experience-center` @ `6c88828`; Draft PR #143 vs `master`; no GitHub PR checks configured (not a product bug); adversarial review found no new RACES defects; G5.5 no code change.
- 2026-08-17 merge: PR #143 marked ready and merged @ `d4f9210303aac5f6ae964f0259fb9fc25dd59743`. Post-merge smoke green. Plan `status: done`. Next branch: `feat/races-investigation-execution-ux`.
