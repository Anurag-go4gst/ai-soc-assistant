...
- [x] **G1 — Close Plan 4**
  ...
  - **Evidence:** **PLAN 4 CLOSED 2026-08-12 — 19/19, 0 audit gaps.** Every gate re-run fresh at closure.
    ...
    **Six report artifacts regenerated by the governance run were reverted, not committed** — `langgraph_dual_parity_report.json` / `_summary.md`, `soc_clean_answer_eval_report.json` / `.csv` / `_summary.md`, `llm_template_audit_report.md`. They are outputs, not gates. Their committed copies were last generated at **`322c2bcd`, 2026-07-25** — before Plans 2, 3 and 4 — so regenerating them absorbs roughly two and a half weeks of accumulated behavior change, only a fraction of which is Plan 4's. Committing them under a Plan 4 closure would have silently attributed that drift to this plan. Reverted; manifest re-checked `14 checked, unchanged`. **Refreshing them is a separate task with its own attribution question**, and is deliberately not done here.
...
| 2026-08-12 | **MERGED — PR #130, merge commit `2f678b9712d091b772d5ea160a2a81e54cde9e24`, `--merge` not squash (34 commits preserved).** This is the **new authoritative baseline**, superseding `93562c1`. Pre-merge cumulative review over `93562c1..77e89f7`: 27 files, `+8168/-121`; goldens, governed registries and all six pre-existing protected baselines **UNCHANGED**; only the approved `BR-a`/`RF-a`/`CW-a` artifact updates present; execution flag still `false`; no capability widening; no unrelated user-owned dirt in the range; the six stale governance reports correctly excluded. `question_runtime_map_v1.json` was re-serialized to the committed `indent=2` format first, shrinking a 9-field change from a 14718-line diff to **9+/9-**. Post-merge: local master fast-forwarded, `origin/master` in sync, protected manifest `14 checked, unchanged`. |
