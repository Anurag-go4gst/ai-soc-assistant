...
| **Checklist** | **15 checked** / 27 unchecked (through **3.1**) |
| **Next item** | **3.2** — Preserve J7 evidence authority (regression pin) |
| **0.2 note** | `DONE_WITH_ENVIRONMENT_UNRESOLVED_INPUTS` — two production traces unavailable; design-case diagnostic only |
| **Product tip** | RQC `requested_conditional_actions` shipped in `09c3f97a`; pending-intent display `67246132`; 3.1 contract-gate change is the current item commit |
