...
> **🔴 Track D data gap (newly surfaced — blocks B9 usefulness):** there is **no precomputed, offline question→`use_case_id` mapping** in any non-runtime source. Question→use_case resolution happens only at runtime inside the router (`app.*`, which the generator is forbidden to import). Consequence: `use_case_id`, `github_reference_skill`, `github_intake_decision`, and `evidence_requirements` are **null for all 105 rows**. The generator deliberately did **not** reconstruct the router to fake a join (that would make the artifact lie). **Follow-up (new backlog item BL-004):** add a deterministic offline `question_id → use_case_id` map (curated JSON or an exported snapshot from a one-shot runtime dump), then the use-case-gated columns and the GitHub-intake join become populated. Until then the matrix is a valid **scaffold** with router-skill + template-status signal only.
