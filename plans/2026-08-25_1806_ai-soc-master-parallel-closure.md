...
| P8 | F EVAL | CODEX | EVALUATED_FAIL | `b6e4befe` | working tree on `6e7e379c` | P2/P4/P5 | P8-A/B/C/D complete | A/B no gated gain; journeys 7/8 J7 fail; browser exercised | P9 blocked | MODEL_CAPABILITY_CANDIDATE t4/spl/planner; J7+ChatPanel protected | NOT_MERGED_AS_DONE |
...
| H-PROMO-02 | Routing audit | `rt.para.011` | ACCEPTED_INHERITED_RESIDUAL | P9 | Operator 2026-08-26 | Not a promotion blocker; not PASS | P9 |
...
| H-PROMO-05 | Promotion | Mac/Linux same candidate SHA | DONE | P9 | Linux 7109/0 matches Mac at c109402d | `linux_backend_attest.txt` | P9 |
| H-PROMO-06 | Freeze audit | RACES baseline and protected changes | DONE | P0.1/P9 | RACES 8/8; freeze probe PASS | P9 remasure | P9 |
| H-PROMO-07 | Governance | Stage 3 governance | ACCEPTED_INHERITED_GOLDEN | P9 | Composite FAIL only at Tier0×2 accepted inherited; other sections PASS | Named residuals | P9 |
...
- [ ] **P8 - L3 live local LLM semantic evaluation**
  - **STATUS:** EVALUATED_FAIL (P8-A/B/C/D complete; MODEL_QUALITY_GATE FAIL after two candidates; JOURNEY J7 FAIL; UI/UX FAIL remaining ChatPanel leakage; P9 blocked)
  ...
  - **Evidence:** LIVE_EVAL_PRODUCT_SHA=`b6e4befe`. L3-1 commit `472ed898`; bank_hash `5f78ccbe1940149a67dcd1052140c44c854ec42a409d7644b47e5357010dbf51` (16 rows) unchanged. L3-2 live run against office `http://10.52.1.13:8004/v1` model `foundation-sec-instruct` (vLLM, root `foundation-sec-8b-instruct`). :8081 still closed. ACTIVE prompts only (`tmpl.semantic_t4`/`tmpl.spl_advisory_generator`/`tmpl.investigation_planner`/`tmpl.shape_advisor` v1.0.0); candidate=null. MCP execution forced off. Thresholds frozen. Official artifacts: `docs/evals/p8_l3/l3_2_raw_results.json`, `l3_2_scorecard.json`, `l3_2_failure_analysis.json`. Harness-only defects H1–H3 fixed then full 16-row rerun (`l3_2_harness_fix.json`); product/prompts/bank/thresholds not edited. Rows executed 16; LLM quality pass 3; failed 8; fallback rescue L3.PL.01. LLM_SUCCESS_RATE 0.3077; SEMANTIC_SUCCESS_RATE 0.2885 (floor 0.5 FAIL); initial_pass_rate 0.20 (floor 0.4 FAIL); schema_validity 0.9375 PASS. AUTHORITY_VIOLATIONS 0; EVIDENCE_HALLUCINATIONS 0. T4 0/4 accepted (validator rejection). SPL rolling/trend/sequence no candidate; ranking denied_traffic loss; raw PASS. Chat review-only + investigate PASS (no live MCP). FU.01 OBSERVED_NOT_SCORED. Planner structured-output failure rescued by deterministic baseline. AB.01 unsupported PASS. PR.01 ACTIVE-only PASS. Latency p50 977 / p95 4233 / max 5170 ms. BASELINE_FAIL_REQUIRES_CANDIDATE. CANDIDATE_PROMPT_CREATED=NO. LIVE_AB_EVAL_PERFORMED=NO. BLOCKED_REASONING_ROLES_ENABLED=NO. LIVE_MCP_USED=NO. P9_ACTION=BLOCKED. Cross-contract after harness-only work: RACES 8; P0 L2 13; LIVE-RQC 10; L2 journeys 28; P8 bank 7; prompt-policy contracts 176. No prompts tuned. PUSH_PERFORMED=NO.
  - **Evidence (2026-08-26 P8-A/B/C/D — do not rewrite L3-2):** Continued from HEAD `6e7e379c` with `b6e4befe` as ancestor (no reset). P8-A registered separate CANDIDATE overlays (`tmpl.*.candidate` `1.2.0-candidate`) without editing ACTIVE; production `prompt_eval_arm` remains `active`; `contract_for_role().candidate` stays null; blocked reasoners unchanged. P8-B fair A/B on frozen bank hash `5f78ccbe…` same office vLLM: ACTIVE rerun matched L3-2 (semantic 0.2885, initial_pass 0.20, T4 0/4, SPL 1/5, planner schema 0); CANDIDATE v1 then v2 identical gated rates; `docs/evals/p8_l3/ab_comparison.json` `NO_PROMOTION` / all three roles `MODEL_CAPABILITY_CANDIDATE`. Authority 0 / evidence hallucinations 0 both arms. P8-C journey bank 8 on ACTIVE: 7 PASS / J7 FAIL (SOP `knowledge_recall` offered Create a remediation plan); execution/evidence overclaims 0; follow-up J8 PASS. P8-D production `/chat` browser at `127.0.0.1:3013`: UI-J1 plan Approve/Edit/Cancel; UI-J2 review-only SPL; UI-J3/J4 remediation offered; chrome EC copy fixed unprotected (title/TopBar/Login/SideNav subtitle); ChatPanel scenario picker remains protected. Full backend `1 failed, 7081 passed, 45 skipped, 6 xfailed` — failure `test_factory_generators_check_against_committed_artifacts` classified GitHub-skill ENVIRONMENT residual, not a new P8 product failure. Frontend 118 passed; `npm run build` green. LIVE_MCP_USED=NO. PUSH_PERFORMED=NO. P8_STATUS=`P8_FAIL_MODEL_CAPABILITY_REVIEW`. P9_ACTION=BLOCKED.
...
- [x] **P9 - Promotion governance and residual failure adjudication**
  - **STATUS:** DONE — `P9_COMPLETE` / `GO_FULL_PROMOTION=YES`. Linux exact-SHA PASS (7109/0). Operator accepted three inherited residuals. `P10_ELIGIBLE_TO_START=YES`; P10 not started.
