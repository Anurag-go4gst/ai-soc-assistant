...
- [x] **0.3** — Classify failure seams (primary + contributing)
  ...
  - **Depends on:** 0.2
  - **Evidence:** Wrote `docs/evals/answer_shape/trace_diagnosis_v1.md`. Primaries: prod#1/#2 = `ENVIRONMENT_UNRESOLVED` (no invented root cause); design-case = `OBJECTIVE_PERSISTENCE` with CONTRIBUTING `[CAPABILITY_SELECTION, ENVIRONMENT_UNRESOLVED]`. Checksum 2+1=3 reviewed. VERIFY_0.3_PASS. Commit: .
...
- [ ] **3.3** — Implement B4 with two distinct eligibility gates end-to-end
...
- [ ] **3.4** — Email draft on Phase 10 / action lane (not ResourcePlan)
...
- [ ] **3.6** — Email draft production (governed)
