...
- [x] **C1-E4 — EXECUTION-DRIVEN: wire both runtimes behind explicit authority**
  ...
  - **Evidence:** **COMPLETE 2026-08-11; runtime commit `d7c4d62`.**

    **Activation posture implemented exactly as approved:** `DEDICATED_DEFAULT_FALSE_FLAG` with the C0-recorded name `AI_SOC_RESOURCE_PLAN_EXECUTION_ENABLED` → `settings.ai_soc_resource_plan_execution_enabled`, **default false**, mirrored in `.env.example` next to the dispatch-v2 flag it most resembles. Tests pin all three: the model-field default is `False`, the env var of that exact name flips it, and `.env.example` declares it `=false`. No `CANONICAL_DEFAULT_AFTER_PROOF` behavior exists — there is no hidden temporary default-on path.

    **One seam, both runtimes.** Wiring is at `build_step_walk_dispatch_schedule` only. Both runtimes converge there: the imperative pipeline calls `execute_plan_dispatch` (`pipeline.py:655`) and the Resource Planner graph node `rp_node_composed_dispatch` calls `graph_node_composed_dispatch`, which is a one-line delegate to the same `execute_plan_dispatch` (`pipeline.py:2273`, asserted by `test_both_runtimes_share_one_dispatch_seam`). No second scheduler exists; `test_dual_runtime_single_orchestration` stays green.

    **The ladder, documented at its call site** (the previous docstring claim "dispatch still follows the legacy stage pipeline" is now false and was replaced): (1) flag off → return the fixed predicate schedule **immediately**, before any model validation, compiler import, or trace annotation — so flag-off parity is *structural*, proven by `test_flag_off_reaches_no_execution_contract_code`, which fails the run if the compiler is called at all; (2) a dispatch-v2 projected schedule exists → that projection owns stage ordering, including live pre-SPL MCP discovery, so the execution-driven path stands down with `dispatch_v2_projected_schedule`; (3) otherwise compile, with every downgrade (absent/empty/invalid/unsupported/unschedulable, plus `plan_parse_failed` for a plan that will not validate) falling back to the identical fixed schedule.

    **Inactive-posture parity:** flag-off compiled-vs-fixed schedules are equal on all four live composed probes, and `plan_dispatch_trace` carries **no** `execution_order` key at all when the flag is off — the new trace field is emitted only on flag-on turns, so a flag-off turn's trace is byte-identical to before this commit. Production parity at the default posture: `total=120 base_105=105 exact=120 approved=0 critical=0`.

    **Flag-on behavior:** the compiled schedule equals the fixed schedule on all four probes — activation changes *which component holds ordering authority*, not the order itself, which is what makes C1-E6's proof tractable. Trace records `execution_order = {"active": true, "downgrade_reason": null}`; the v2-precedence case records `active=false` with the reason.

    **Gate sequence preserved:** flag-on keeps `spl_source_resolve` before `execution` and `execution` last. The execution stage remains the sole owner of `evaluate_mcp_execution` and HIL; `test_mcp_execution_gate` is green. Idempotency is unaffected (`test_execution_idempotency`, `test_per_step_hook_idempotency` green), and on both flag postures the hooks actually invoked equal the trace's `dispatch_schedule` with no duplicate call.

    **Scope recorded honestly.** (a) The COE host runs with dispatch-v2 enabled, so on that host the execution-driven path is active only on turns *without* a v2 projected schedule; integrating the two projections is out of C1-E4 scope. Both directions are tested. (b) The guided-hybrid and session-SPL-refine branches bypass `execute_plan_dispatch` entirely (`pipeline.py:651-655`), so the new path never activates there — C1-E6's matrix should not chase that coverage.

    **Gates.** Exact Verify pytest slice: **111 passed** (`test_resource_plan_execution_wiring.py`, `test_resource_plan_execution_activation.py`, `test_dual_runtime_single_orchestration.py`, `test_dispatch_authority_wiring.py`, `test_resource_plan_step_dispatch.py`, `test_mcp_execution_gate.py`, `test_execution_idempotency.py`, `test_per_step_hook_idempotency.py`). Production parity `--check` as above. Manifest `protected artifacts unchanged (13 checked)`. Invariant check: six groups PASS; group 6 is an **explicitly approved exception** — the new flag is default-false and was approved by exact name in the C0 decision record (`execution_order_flag_name`, Anurag, `2026-08-11T05:53:15Z`), which is the standing "no new flags" policy's override for this one flag. No safety check became flag-disableable and no port changed.
  - **Invariant / manifest:** Full invariant check.
...
- [x] **G0 — Align current architecture/operator documentation with selected outcomes**
  ...
  - **Evidence:** **COMPLETE 2026-08-11; documentation commit `0374972`.** `rg` was run over the six named surfaces before editing; only claims Plan 2 actually changed were touched, and all historical audit evidence was left intact (every edit is an additive dated note or a new closing section, except the two `llm_budget_model.md` table rows whose content became factually false).

    | File | Claim changed |
    |---|---|
    | `docs/architecture/mcp_tool_routing.md` | Added a dated `RETIRE` note: the fenced lane's unreachable call sites were removed (`55ae6a7`), `graph_node_evidence_planning` is still the loop's only initializer and still fails closed, **`MAX_MCP_HOPS` was kept deliberately** because it bounds recipe call budgets (`evidence_loop.py:639`) and is not inert, the three planning-model rails are gone as authorities, and **bounded pre-SPL discovery under dispatch-v2 remains live and is a different mechanism** — the explicit fenced/retired vs live distinction the item requires. |
    | `docs/architecture/chat_pipeline_state_v2_and_node_trace.md` | Added a dated note covering the three A0/A1/C0 changes: topology is runtime-derived (builder fixed edges + mapped conditional destinations + dynamic `Send`) and reconciled by exact equality with `get_graph()` no longer used for edges; `route_setup` removed as a proven orphan; decision-record refs validated/corrected and explicitly **descriptive only, never a scheduling authority**; ResourcePlan order is lineage unless the default-false flag is on. |
    | `docs/architecture/llm_budget_model.md` | Planner-bridge hop row marked retired (no planning-model hop exists). Added an explicit scope block: `AI_SOC_GUIDED_LLM_ENABLED` is **budget/deadline only, gating no planning call**, and the `guided_investigation_plan_llm` dispatch-step label no longer denotes a model hop and is not emitted — verified by `rg` showing no production emitter, only the explanatory comment at `pipeline.py:5883`. |
    | `docs/architecture/architecture_review_2026-08-08.md` | New closing **Plan 2 outcomes (2026-08-11)** section: B1 `RETIRE` with what was retired, kept, and kept-for-a-stated-reason plus the honestly recorded one-round refinement limitation; the topology/decision-record corrections including the union that had masked two fabricated edges; and C0 `EXECUTION-DRIVEN` with the measured lineage finding, the contract/compiler/handoff/reconcile surface, the activation posture, dispatch-v2 precedence, and the guided-hybrid/session-refine bypass. |
    | `CLAUDE.md` | Two boundary lines (no planning-model hop + retired-rail prohibition; ResourcePlan order/activation semantics) and the Plan 2 row in the plan index. |

    `docs/coe/COE_ROLLOUT_CONFIGURATION.md` was checked and **not** edited: it carries no per-flag table containing the comparable dispatch-v2 flag, so there was nothing to keep in sync; `.env.example` already documents the new flag, and the flag-rightsizing disposition table carries its row (`9e41c4e`).

    **Gates.** `pytest backend/app/tests/test_architecture_details_flow_contract.py -q` → **7 passed**. `cmp -s docs/architecture/details.html frontend/public/docs/architecture/details.html` → identical; **no published mirror changed**, since no current-path diagram claim changed, so the protected `published_doc_mirrors` group needed no recapture and none was taken. `npm run build` in `frontend/` completed including `postbuild` chmod, producing no tracked change. Manifest `protected artifacts unchanged (13 checked)`. Documentation-only invariant review: no runtime, flag, or authority change; no secrets; no eval/golden/registry refresh.
...
- [x] **G1 — Final Plan 2 closure gate**
  ...
  - **Evidence:** **COMPLETE 2026-08-11.**

    **27/27 disposition.** Plan audit reports **27 checked, 0 unchecked, 0 gap(s)**. Of the 27: **19 executed with observed evidence** (P0, A0, A1.1, A1.2, B0, B1, B2-R1…R4, C0, C1-E1…E6, G0, G1) and **8 dispositioned `N/A` against an explicit decision** — the seven `B2-W*` items plus `C1-L`, each carrying the decision, approver and timestamp (`N/A — rejected by B1 decision RETIRE, Anurag / 2026-08-10T17:29:24Z` and `N/A — rejected by C0 decision EXECUTION-DRIVEN, Anurag / 2026-08-11T05:53:15Z`). No item is checked without evidence, and no `N/A` lacks a decision reference.

    **Selected decisions:** B1 `RETIRE` (`guided_hybrid_llm_rail_disposition: RETIRE_PROPOSER`), Anurag, `2026-08-10T17:29:24Z`. C0 `EXECUTION-DRIVEN`, Anurag, `2026-08-11T05:53:15Z`, with `activation_posture: DEDICATED_DEFAULT_FALSE_FLAG`, `execution_order_flag_name: AI_SOC_RESOURCE_PLAN_EXECUTION_ENABLED` (default false), and `v1_v2_posture: EXTEND_LIVE_RESOURCE_PLAN`.

    **Final gate results, all re-run at closure:**

    | Gate | Result |
    |---|---|
    | Plan-discipline audit | `27 checked, 0 unchecked, 0 gap(s)` |
    | Protected manifest (before and after) | `protected artifacts unchanged (13 checked)` |
    | Reference probes `--check` | **10/10**, all match the frozen baseline |
    | Production parity `--check` | `total=120 base_105=105 exact=120 approved=0 critical=0` |
    | Selected targeted suites (`-k` closure filter) | **182 passed, 4804 deselected** |
    | Governance regression | **`stage3_governance_regression: PASS`** — harness 6/6, dual parity `120 exact / 0 approved / 0 critical`, clean-answer `120 pass / 0 review / 0 fail / 0 critical / 0 major / 0 display`, Cisco power grid `PASS=50 / REVIEW=0 / FAIL=0 / CRITICAL=0`, backend `4978 passed, 2 skipped, 6 xfailed` |
    | Independent full backend pytest | **`4978 passed, 2 skipped, 6 xfailed, 2 warnings in 553.65s`**, 0 failed |
    | Frontend build (G0 touched docs) | `npm run build` exit 0 including `postbuild` chmod; no tracked change, mirrors still byte-identical |

    **Cumulative diff review (`c6d900a..HEAD`, 15 commits, 20 files, +2779/−4).** Scanned for secrets: the only matches for `password`/`token`/`secret`/`api_key` in added lines are a probe question about contractor password policy and the *negative* assertion list that forbids such names in handoff keys. Scanned for authority expansion: the only `splunk_run_query` occurrences are test-fixture `resource_id` labels, and the only `call_tool`/`evaluate_mcp_execution` occurrences are in a forbidden-call allowlist asserting their absence. No baseline, golden answer, or governed registry was refreshed — governance regenerated five `docs/evals/` reports at both C1-E6 and G1; each time the verdict/classification row projections were byte-equal and summary counts unchanged (only `generated_at` differed), so the files were reverted. No unrelated dirt was committed; the pre-existing user-owned files (`.claude/settings.local.json`, `.playwright-mcp/`, two G0 PNGs, `output/`) are untouched.

    **Cumulative invariant check: 7/7 PASS.** (1) No LLM→MCP path; the retired planning rails stay retired and every new module is AST-pinned free of `app.connectors`/`app.mcp`/`app.llm`. (2) Candidate SPL still cannot reach execution; only approved non-empty `spl_validation.normalized_spl` satisfies the gate handoff, and SPL validation precedes the execution stage on both flag postures. (3) `app/demo/` untouched. (4) No secrets; no handoff key names a credential-shaped field. (5) No new state channel; the single new trace key is emitted only when the flag is on, so flag-off traces are byte-identical; both runtimes share one dispatch seam. (6) One new flag, default false, approved by exact name in the C0 record — the standing "no new flags" policy's explicit override; no safety check became flag-disableable; ports unchanged. (7) No test weakened or deleted; every correction was to a *new* test's own expectation and is documented in the owning item's evidence.

    **Commits (`c6d900a` → `HEAD`):** `e9153e0` C0 decision · `78bd76c` + `2eff97b` C1-E1 · `4b10fc3` + `62dd4d0` C1-E2 · `e0bada3` + `6d943e2` C1-E3 · `d7c4d62` + `529c737` C1-E4 · `b2ccf64` + `0f9c071` C1-E5 · `9e41c4e` flag-disposition fix · `61fe4ec` C1-E6 · `0374972` G0 · `1c50a7d` G0 evidence.

    **Known gaps carried forward, none blocking closure:**
    1. **Guided refinement is still one-round in production.** C1-E3 built the round-varying mechanism (`refinement_decision` over unresolved evidence gaps) and proved it, but wiring it into guided investigation was outside C1-E3's boundary. `MAX_GUIDED_INVESTIGATION_ROUNDS` remains the hard bound and no retired LLM proposer may be restored to close this.
    2. **Execution-driven order stands down under dispatch-v2.** On hosts with dispatch-v2 enabled (including COE), a projected schedule wins wherever it exists, so the compiled path applies only to turns without one. Integrating the two projections is a separate scoped change.
    3. **Guided-hybrid and session-SPL-refine bypass the seam entirely**, so ResourcePlan-driven ordering never applies to them.
    4. **Pre-existing latent bug, still unfixed and still out of scope** (recorded at A0): `pipeline.py:3669` calls `.get()` on `_query_signals_from_state()`, which returns `None` when `query_to_intent` is absent, raising `AttributeError` instead of degrading. It needs its own correctness item.
    5. **Flag-on changes authority, not order.** No approved order delta was taken, by design; any future intentional reorder must be proven against the C1-E6 matrix as its baseline.
...
| 2026-08-11 | **PLAN 2 CLOSED at G1 — 27/27, 0 gaps.** 19 items executed with evidence, 8 dispositioned N/A against the two decision gates (B2-W1…W7, C1-L). Final gates: plan audit `27 checked / 0 unchecked / 0 gap(s)`, manifest 13/13, reference probes 10/10, parity `120 exact / 0 approved / 0 critical`, targeted closure suites 182 passed, governance PASS with backend `4978 passed / 0 failed`, independent full backend `4978 passed / 0 failed`, frontend build clean. Cumulative diff `c6d900a..HEAD` = 15 commits / 20 files / +2779−4, scanned clean for secrets and authority expansion; no baseline refreshed. Five known gaps recorded in G1 evidence, none blocking: guided refinement is built but unwired, execution-driven order stands down under dispatch-v2, guided-hybrid/session-refine bypass the seam, the pre-existing `pipeline.py:3669` degrade bug still needs its own item, and flag-on changes authority rather than order by design. Frontmatter set `status: done` / `implementation_readiness: COMPLETE`. |
