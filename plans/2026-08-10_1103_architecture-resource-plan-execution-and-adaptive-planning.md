...
- [ ] **P0 — Freeze and freshly verify the post-G1 baseline**
  ...
  - **Evidence:** _(paste baseline→HEAD allowed path list, empty runtime worktree-path result, exact HEAD/status, 13-artifact result, 10 probe rows, parity tuple, governance/backend counts, safe host flags, topology counts, record count, and before/after status)_
...
- [ ] **B2-R1 — RETIRE: pin deterministic canonical behavior and the live pre-SPL boundary**
  ...
  - **Failing-first / observation:** New test must capture current shadow call, fenced bridge, direct imperative guided proposer, legacy fence, deterministic guided behavior, and live pre-SPL distinction before removal; the post-R2 absence assertions fail first.
  ...
  - **Evidence:** _(three-surface inventory/call counts, deterministic fingerprints, guided-proposer versus guided-execution distinction, live pre-SPL distinction assertions, pytest, manifest, invariant, commit)_
  - **Invariant / manifest:** Full invariant check; no runtime behavior change in this test-first commit.
  - **Commit boundary:** Tests only.
  - **Stop:** Live pre-SPL discovery cannot be isolated; current canonical output differs from P0.
...
- [ ] **B2-R2 — RETIRE: remove the discarded shadow call and unreachable promotion surfaces**
  ...
  - **Depends on:** B2-R1.
  - **Failing-first / observation:** The B2-R1 test must fail if either runtime retains a planning-model call or if deterministic guided behavior is accidentally removed. Consumer inventory is mandatory before deletion.
  ...
  - **Commit boundary:** Shadow/bridge retirement only; no legacy discovery deletion.
